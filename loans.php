...
$sql = "SELECT a.*, b.account_name, b.nin_number FROM bank_loans a LEFT JOIN bank_accounts b on a.account_number=b.account_number";
$response=array();
$res= $conn->query($sql);
...
while($row=$res->fetch_assoc()){
	...
	$response[$x]['nin_number']=$row['nin_number'];
                                        ...
                                        ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="center-align">
                                                	...
                                                	<a href="#editLoan_<?= $acct['id']; ?>" class="modal-trigger"><i class="material-icons pink-text">edit</i></a>
                                                </td>
                                            </tr>
                                            <?php } ?>
...
<?php
    $loans=$conn->query("SELECT a.*, b.account_name FROM bank_loans a LEFT JOIN bank_accounts b on a.account_number=b.account_number");
    while ($loan = $loans->fetch_assoc()) { ?>
    <div id="editLoan_<?= $loan['id']; ?>" class="modal">
        <div class="modal-content">
            <h4>Loans</h4>
            <div id="basic-form" class="card card card-default scrollspy">
                <div class="card-content">
                    <h4 class="card-title">Add New Loan</h4>
                    <form action="loans.php?edit" method="POST">
					<input type="hidden" name="TABLE" value="bank_loans" >
					<input type="hidden" name="account_id" value="<?= $loan['id']; ?>" >
                        <div class="row">
                            <div class="input-field col s12">
								<select class="select2 browser-default" name="data[account_number]">
									...
									{
									?>
									<option value="<?= $acct['account_number']; ?>"  <?php echo ($loan['account_number']==$acct['account_number'])?"selected":""; ?> >
                        ...
                        <div class="row">
                            <div class="input-field col s12">
                                <input id="account" type="text" name="data[description]" value="<?= $loan['description']; ?>">
                                <label for="account">Loan Identifier</label>
                            </div>
                        </div>
                        <div class="row">
                            <div class="input-field col s12">
                                <input id="Branch" type="text" name="data[amount]" value="<?= $loan['amount']; ?>">
                                <label for="Branch">Amount</label>
                            </div>
                        </div>
                        <div class="row">
                            <div class="input-field col s12">
                                <input id="Branch" type="text" name="data[due_date]" class="datepicker" value="<?= date("d/m/Y", strtotime($loan['due_date'])); ?>">
                                <label for="Branch">Due Date</label>
                            </div>
                        </div>
                        <button class="btn cyan waves-effect waves-light right" type="submit" name="action">Submit
